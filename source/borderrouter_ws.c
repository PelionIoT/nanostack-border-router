...
#define WS_DEFAULT_NETWORK_NAME "ARM-WS-TESTING"
...
static void mesh_network_up()
{
    ...
    int status;
    int8_t wisun_if_id = wisun_br_conn_handler_wisun_interface_id_get();
...
#else
    ws_conf.network_name = malloc(sizeof(WS_DEFAULT_NETWORK_NAME)+1);
    strcpy(ws_conf.network_name, WS_DEFAULT_NETWORK_NAME);
...
#else
    ws_conf.uc_channel_function = 255;
...
#else
    ws_conf.uc_fixed_channel = 0xffff;
...
#else
    ws_conf.bc_fixed_channel = 0xffff;
...
void wisun_rf_init()
{
    ...
    if (rf_driver_id>=0) {
        ...
        int wisun_if_id = arm_nwk_interface_lowpan_init(mac_api, ws_conf.network_name);
        wisun_br_conn_handler_wisun_interface_id_set(wisun_if_id);
    }
}
...
static int wisun_interface_up(void)
{
    ...
    int wisun_rf_id = wisun_br_conn_handler_wisun_interface_id_get();

    fhss_timer_ptr = &fhss_functions;
    
    ret = ws_management_node_init(wisun_rf_id, ws_conf.regulatory_domain, ws_conf.network_name, fhss_timer_ptr);
    if (0 != ret) {
        tr_error("\r WS node init fail - code %"PRIi32"", ret);
        return -1;
    }

    if (ws_conf.uc_channel_function != 255) {
        ret = ws_management_fhss_unicast_channel_function_configure(wisun_rf_id, ws_conf.uc_channel_function, ws_conf.uc_fixed_channel, ws_conf.uc_dwell_interval);
        if (ret != 0) {
            tr_error("Unicast channel function configuration failed %"PRIi32"", ret);
            return -1;
        }
    }
    if (ws_conf.bc_channel_function != 255 ||
        ws_conf.bc_dwell_interval != 0 ||
        ws_conf.bc_interval != 0) {
        ret = ws_management_fhss_broadcast_channel_function_configure(wisun_rf_id, ws_conf.bc_channel_function, ws_conf.bc_fixed_channel, ws_conf.bc_dwell_interval, ws_conf.bc_interval);
        if (ret != 0) {
    ...
    if (ws_conf.uc_dwell_interval != 0 ||
        ws_conf.bc_dwell_interval != 0 ||
        ws_conf.bc_interval != 0 ) {
        ret = ws_management_fhss_timing_configure(wisun_rf_id, ws_conf.uc_dwell_interval, ws_conf.bc_interval,ws_conf.bc_dwell_interval);
        if (ret != 0) {
            tr_error("fhss configuration failed %"PRIi32"", ret);
            return -1;
        }
    }
    if (ws_conf.regulatory_domain != 255 ||
        ws_conf.operating_mode != 255 ||
        ws_conf.operating_class != 255 ) {
        ret = ws_management_regulatory_domain_set(wisun_rf_id, ws_conf.regulatory_domain, ws_conf.operating_class, ws_conf.operating_mode);
        if (ret != 0) {
    ...
    ret = arm_nwk_interface_up(wisun_rf_id);
    if (ret != 0) {
...
static int backhaul_interface_up(int8_t driver_id)
{
    ...
    int8_t backhaul_if_id = wisun_br_conn_handler_eth_interface_id_get();
    if (backhaul_if_id != -1) {
        ...
        if (backhaul_if_id >= 0) {
            ...
            wisun_br_conn_handler_eth_interface_id_set(backhaul_if_id);
...
{
    int retval = -1;
    if (wisun_br_conn_handler_eth_interface_id_get() != -1) {
        arm_nwk_interface_down(wisun_br_conn_handler_eth_interface_id_get());
        wisun_br_conn_handler_eth_interface_id_set(-1);
        wisun_br_conn_handler_ethernet_connection_update(false);
        retval = 0;
    }
    else {
...
static void borderrouter_tasklet(arm_event_s *event)
{
    ...
    switch (event_type) {
        case ARM_LIB_NWK_INTERFACE_EVENT:
            ...
            if (event->event_id == wisun_br_conn_handler_eth_interface_id_get()) {
...
static void network_interface_event_handler(arm_event_s *event)
{
    ...
    switch (status) {
        case (ARM_NWK_BOOTSTRAP_READY): { // Interface configured Bootstrap is ready
            ...
            connectStatus = true;
            tr_info("BR interface_id: %d", wisun_br_conn_handler_eth_interface_id_get());
            if (-1 != wisun_br_conn_handler_eth_interface_id_get()) {
                // metric set to high priority
                if (0 != arm_net_interface_set_metric(wisun_br_conn_handler_eth_interface_id_get(), 0)) {
                ...
                if (backhaul_bootstrap_mode == NET_IPV6_BOOTSTRAP_STATIC) {
                    ...
                    arm_net_route_add(backhaul_route.prefix,
                                      ...
                                      wisun_br_conn_handler_eth_interface_id_get());
                }
                tr_info("Backhaul interface addresses:");
                print_interface_addr(wisun_br_conn_handler_eth_interface_id_get());
                wisun_br_conn_handler_ethernet_connection_update(connectStatus);
            }
            break;
        }
...
static void wisun_interface_event_handler(arm_event_s *event)
{
    ...
    switch (status) {
        case (ARM_NWK_BOOTSTRAP_READY): { // Interface configured Bootstrap is ready
            ...
            if (arm_net_interface_set_metric(wisun_br_conn_handler_wisun_interface_id_get(), MESH_METRIC) != 0) {
    ...
    wisun_br_conn_handler_wisun_connection_update(connectStatus);
}
